...
\- **Removing low-count guides**: guides with very few reads across all samples may not provide reliable information. By default, guides with a sum of \<30 counts across all samples are filtered out.
...
Once the model is finished fitting, `biscuit_fit()` converts the original `dough` object into a `biscuit` object, which contains the original input data from `dough$data` under `$data` and the posterior samples under `fit`.
