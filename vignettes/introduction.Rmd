...
`biscuit` requires `cmdstanr` to run. To install `cmdstanr` (and `cmdstan`):
...
To demonstrate how to use `biscuit`, we provide an example using the data from Regnase-1 screen [1]. The Regnase-1 screen is a bi-directional CRISPR screen targeting 3018 genes, with about 6 guides per gene with one control sample and three treatment samples.
...
`biscuit` performs the following pre-processing steps:
...
Filtering can also be done in in the same step as fitting the model (see below), but only with default configurations.
...
biscuit_fit <- bake_biscuit(
  dough,
  output_dir,
  filter = FALSE, # here, we indicate false as we filtered before (see above)
  save_samples = FALSE,
  n_parallel_chains = 4,
  seed=13,
  pseudocount=TRUE)
...
Once the model is finished fitting, `biscuit_fit()` converts the original `dough` object into a `biscuit` object, which contains the original input data from `dough$data` under `$data` as well as the sampling diagnostics and run time under `$fit`.

### Explore posterior estimates

`biscuit` computes posterior summaries as well as the local false discovery rate for each guide-level and gene-level effect for downstream analysis.

#### Summarizing posterior estimates

`biscuit` computes a summary table of the posterior for the model’s parameters of interest:

-   `mu`: gene-level effects
-   `beta1`: guide-level effects
-   `beta0`: baseline guide-level expression (intercept)
-   `phi`: guide-level dispersion
-   `eff`: guide-level efficacy

For each parameter, the summary table reports the **mean**, **median**, **standard deviation**, and **2.5% and 97.5% quantiles**. In addition, the **local false dicovery rate** is calculated for `beta1` and `mu`, providing a measure of significance as a function of confidence.
