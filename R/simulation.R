...
#' @export
make_playdough <- function(n_genes, guides_per_gene,
                           n_control = 2, n_treatment = 2,
                           p_effects = 0.2, p_positive = 0.1,
  ...
  if(is.null(counts) & effect_mode == 'empirical') {stop('no counts provided for empirical estimation')}
  ...
  if (!is.null(counts)) { # if provided with counts, calculate empirical estimates
  ...
  if (effect_mode == "fixed") { # fixed gene effect (one numeric value)
  ...
  } else if (effect_mode == "parametric") { # parametric gene effect (distribution with given parameters)
